...
const WHATSAPP_NUMBER = process.env.WHATSAPP_NUMBER || "919132622222";

const db = new Database(path.join(__dirname, "gharana.db"));
...
if (!count) {
  ...
  [
    ...
    "Cookies",
    "Chocolates"
  ].forEach((name, i) => addCat.run(name, i));

  const cats = db.prepare("SELECT id, name FROM categories").all();
  const cat = Object.fromEntries(
    cats.map((item) => [item.name, item.id])
  );

  const addItem = db.prepare(`
    INSERT INTO items
    (category_id, name, price, description, image, veg, available, sort_order)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
...
app.get("/api/admin/menu", ownerGuard, (req, res) => {
  const categories = db
    .prepare("SELECT * FROM categories ORDER BY sort_order, id")
    .all();
...
app.post("/api/admin/categories", ownerGuard, (req, res) => {
...
app.put("/api/admin/categories/:id", ownerGuard, (req, res) => {
  const name = String(req.body.name || "").trim();
...
app.delete("/api/admin/categories/:id", ownerGuard, (req, res) => {
  ...
  if (used) {
    return res.status(400).json({
      error: "Move/delete the items in this category first."
    });
  }
...
app.post("/api/admin/items", ownerGuard, (req, res) => {
  const b = req.body;
  ...
  const info = db
    .prepare(`
      INSERT INTO items
      (category_id, name, price, description, image, veg, available)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `)
    .run(
      categoryId,
...
app.put("/api/admin/items/:id", ownerGuard, (req, res) => {
  const b = req.body;
...
app.delete("/api/admin/items/:id", ownerGuard, (req, res) => {
  db.prepare(
