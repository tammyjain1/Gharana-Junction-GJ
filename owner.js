...
function openItemModal(it) {
  ...
  document.getElementById('itemForm_imageFile').value = '';
  document.getElementById('uploadStatus').textContent = 'Choose a photo from your phone or laptop.';
...
// ---- Photo upload ----
document.getElementById('itemForm_imageFile').addEventListener('change', async (e) => {
  ...
  if (file.size > 5 * 1024 * 1024) {
    statusEl.textContent = 'Image too large — please choose one under 5MB.';
    return;
  }
  ...
  try {
    const path = `item-photos/${Date.now()}_${file.name}`;
    ...
    task.on('state_changed', snap => {
      ...
      document.getElementById('itemForm_image').value = url;
...
document.getElementById('saveItemBtn').addEventListener('click', async () => {
  if (isUploadingImage) { showToast('Please wait for the photo to finish uploading'); return; }
